...
PrepareCanvas <- function (x, grid_on_top) {
  ...
  canvas <- ggplot(x, aes(x = Year, y = Age)) +
...
PlotMx <- function (x, cont, grid_on_top) {
  ...
  if (cont == FALSE) {
    ...
    #   PrepareCanvas(x, grid_on_top) +
      # discrete heatmap
      indices = which(rawData$cleanedData$State=="Alabama")
      x = rawData$cleanedData[indices,]
      indices = which(x$Sex=="Female")
      x = x[indices,]
      indices = which(x$Age=="Allage")
      x = x[-indices,]
      indices = which(x$directCost==0)
      x = x[-indices,]
      canvas <- ggplot(x, aes(x = Year, y = Age))
      canvas = canvas + geom_raster(aes(fill = directCost), na.rm = TRUE) 
      canvas
  ...
  if (cont == TRUE) {
    ...
    plot_mx <-
      ...
      canvas = canvas + scale_fill_gradientn(fill_label,
                           colours = cpal_mx,
